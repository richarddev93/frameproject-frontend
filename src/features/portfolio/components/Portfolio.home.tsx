...
export const Portfolio = ({ data }: { data: PortfolioProject[] }) => {
  ...
  const categories = useMemo(() => {
    const unique = Array.from(new Set(data.map((project) => project.category).filter(Boolean)));
    return ["Todos", ...unique];
  }, [data]);

  const filteredProjects = useMemo(() => {
    if (selectedCategory === "Todos") return data;
              ...
              <div className="p-8">
                ...
                <div className="flex flex-wrap gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-white ml-2">{getProjectYear(selectedProject)}</span>
                  </div>
                </div>
              </div>
            </motion.div>
